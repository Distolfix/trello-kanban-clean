...
const Index = () => {
  ...
  const [boardLogo, setBoardLogo] = useState<string>(() => {
    const savedLogo = localStorage.getItem('kanban-board-logo');
    return savedLogo || "";
  });
  const [boardFavicon, setBoardFavicon] = useState<string>(() => {
    const savedFavicon = localStorage.getItem('kanban-board-favicon');
    return savedFavicon || "";
  });
  const [isBoardSettingsOpen, setIsBoardSettingsOpen] = useState(false);

  // Update favicon when boardFavicon changes
  useEffect(() => {
    const favicon = document.querySelector("link[rel*='icon']") as HTMLLinkElement;
    if (boardFavicon) {
      if (favicon) {
        favicon.href = boardFavicon;
      } else {
        const newFavicon = document.createElement('link');
        newFavicon.rel = 'icon';
        newFavicon.href = boardFavicon;
        document.head.appendChild(newFavicon);
      }
    } else {
      // Reset to default favicon when removed
      if (favicon) {
        favicon.href = '/favicon.ico'; // Default favicon path
      }
    }
  }, [boardFavicon]);
  ...
  const handleBoardSettingsChange = (settings: { title: string; logo: string; favicon: string }) => {
    setBoardTitle(settings.title);
    localStorage.setItem('kanban-board-title', settings.title);

    setBoardLogo(settings.logo);
    if (settings.logo) {
      localStorage.setItem('kanban-board-logo', settings.logo);
    } else {
      localStorage.removeItem('kanban-board-logo');
    }

    setBoardFavicon(settings.favicon);
    if (settings.favicon) {
      localStorage.setItem('kanban-board-favicon', settings.favicon);
    } else {
      localStorage.removeItem('kanban-board-favicon');
    }

    setIsBoardSettingsOpen(false);
    toast({
      title: "Impostazioni aggiornate",
      description: "Le impostazioni della board sono state salvate"
    });
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="h-[calc(100vh-4rem)]">
        <KanbanBoard
          ...
          boardLogo={boardLogo}
          onBoardSettingsChange={handleBoardSettings}
      ...
      <BoardSettingsModal
        ...
        onSave={handleBoardSettingsChange}
        currentSettings={{
          title: boardTitle,
          logo: boardLogo || "",
          favicon: boardFavicon || ""
        }}
      />
    </div>
  );
