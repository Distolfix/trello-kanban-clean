...
app.post('/api/cards/:cardId/attachments', upload.array('files', 5), async (req, res) => {
  try {
    ...
    console.log('📁 Upload request received for card:', cardId);
    ...
    console.log('✅ Upload successful, returning attachments:', newAttachments.length);
...
app.post('/api/cards/:cardId/attachments/chunk', chunkUpload.single('chunk'), async (req, res) => {
  try {
    const { cardId } = req.params;
    const { uploadId, chunkIndex, totalChunks, fileName, fileSize } = req.body;

    console.log(`📦 Received chunk ${chunkIndex}/${totalChunks - 1} for ${fileName}`);
...
app.post('/api/cards/:cardId/attachments/finalize', async (req, res) => {
  try {
    const { cardId } = req.params;
    const { uploadId, fileName } = req.body;

    console.log(`🔄 Finalizing upload for ${fileName} with ID ${uploadId}`);
    ...
    const chunkFiles = fs.readdirSync(chunksDir)
      ...
      .sort((a, b) => {
        const aIndex = parseInt(a.split('-chunk-')[1]);
        const bIndex = parseInt(b.split('-chunk-')[1]);
        return aIndex - bIndex;
      });

    if (chunkFiles.length === 0) {
      return res.status(400).json({ success: false, error: 'No chunks found for this upload' });
    }
    ...
    const finalFilePath = path.join(__dirname, 'uploads', 'attachments', finalFileName);
    ...
    const attachment = {
      ...
      size: stats.size,
      mimetype: 'application/octet-stream', // Will be determined by browser
      uploadDate: new Date().toISOString(),
      url: `/uploads/attachments/${finalFileName}`
    };

    // Get current card and update with new attachment
    const card = dbService.getCard(cardId);
    if (!card) {
      // Clean up file
    ...
    console.log(`✅ Upload finalized: ${fileName} (${(stats.size / 1024 / 1024).toFixed(2)}MB)`);
